...
function draw() {
    ...
    scale(width / GRID_WIDTH, height / GRID_HEIGHT);
...
function showStartScreen() {
    noStroke();
    fill(32);
    rect(2, GRID_HEIGHT / 2 - 5, GRID_WIDTH - 4, 10, 2);
    fill(255);
    text(
...
function startGame() {
    ...
    for (let x = X_START; x < X_START + STARTING_SEGMENTS; x += 1) {
        ...
        let segmentPosition = createVector(x, Y_START);
...
function gameOver() {
    noStroke();
    fill(32);
    rect(2, GRID_HEIGHT / 2 - 5, GRID_WIDTH - 4, 12, 2);
    fill(255);
